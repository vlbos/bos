...
fc::variant notify_plugin_impl::deserialize_action_data(action act)
{
  ...
  FC_ASSERT(serializer.valid() && name(serializer->get_action_type(act.name)) != action_name(),
  ...
  try
  {
    ...
    if (options.count("notify-filter-on"))
    {
      ...
      for (auto &s : fo)
      {
        ...
        EOS_ASSERT(fe.receiver, fc::invalid_arg_exception, "Invalid value ${s} for --notify-filter-on", ("s", s));
