   ...
   public:
      ...
      int transaction_size() {
       ...
       void get_action_sequence(uint64_t& seq){
           seq = context.next_global_sequence();
      }
...
public:
   ...
   int bpsig_action_time_seed(array_ptr<char> sig, uint32_t siglen) {
      ...
      for (auto& extension: block_state->block->block_extensions) {
         ...
         if ( act_parts[0] != context.next_global_sequence()-1) continue;
      ...
      if (sig_size <= siglen) {
         ...
         if (sign) {
			...
			uint64_t seq = context.next_global_sequence()-1;
            char* act_parts = reinterpret_cast<char*>(&seq);
...
private:
   vector<uint32_t> action_timestamp() {
	  ...
	  uint64_t seq = context.next_global_sequence()-1;
      uint32_t* act_parts = reinterpret_cast<uint32_t*>(&seq);
